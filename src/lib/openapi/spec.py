...
class OpenapiRouteContent:
    description: str
    content: typing.Dict[str, OpenapiRouteContentSchema]
        ...
        if content_type not in self.content:
            ...
            if 'oneOf' in current_content.schema:
                ...
                self.content[content_type] = OpenapiRouteContentSchema(
                    {'oneOf': [schema, current_content.schema]}
                )
        return self
    ...
    def merge(self, content: OpenapiRouteContent) -> OpenapiRouteContent:
        ...
        if content.content is not None:
            for content_type in content.content:
                target_content = content.content[content_type]
                self.add_schema(content_type, target_content.schema)
        elif content.description is not None:
...
class OpenapiRoute:
    ...
    def merge(
        ...
        self.responses = self.responses | route.responses
        return self
